...
def run(
    ...
    # Lấy thông tin chiều rộng và chiều cao của video
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_txt :
                            ...
                            #chỉ lưu vào 1 file text
                            with open(txt_path + '.txt', 'a') as f:
                                # f.write(('%g ' * 9 + '\n') % (frame_idx + 1, id, bbox_left,  # MOT format
                                #                                bbox_top, bbox_w, bbox_h, 1, 1, 1))
                                f.write(f'{frame_idx+1},{int(id)},{int(bbox_left)},{int(bbox_top)},{int(bbox_w)},{int(bbox_h)},1,1,1\n') #MOT Format
                                # f.write(f'{frame_idx+1},bee,{int(id)},{int(bbox_left)},{int(bbox_top)},{int(bbox_w)},{int(bbox_h)}\n') # format chuan theo Huy


                            # #lưu vào nhiều file text để gắn nhãn giống label image
                            # if frame_idx+1 > 600:
                            #     sys.exit("exit")
                            # x_center = (bbox_left+bbox_w/2)/width_img
                            # y_center = (bbox_top+bbox_h/2)/height_img
                            # w_norm   = bbox_w/width_img
                            # h_norm   = bbox_h/height_img
                            # with open(txt_path + "_"*((frame_idx+1)//10)+str(frame_idx+1)+".txt", "a+") as f:
                            #     f.write(str(int(id)) + " "+ str(x_center)+" "+str(y_center)+" "+str(w_norm)+" "+str(h_norm)+"\n")
